#!/usr/bin/env python3
"""
Load DC Code section similarities from NDJSON into the database.
...
class SimilarityLoader:
    ...
    def load_similarities(self):
        """Load similarities from NDJSON file into database."""
        ...
        try:
            ...
            with open(self.input_file, 'r') as f:
                ...
                pbar = tqdm(
                    ...
                    desc="Loading similarities",
                    unit="pairs"
                )

                for line in f:
                    try:
                        similarity = json.loads(line)
    ...
    def _insert_batch(self, cursor, batch):
        ...
        sql = """
            INSERT INTO dc_section_similarities (section_a, section_b, similarity)
            VALUES (%(section_a)s, %(section_b)s, %(similarity)s)
        ...
        try:
            ...
            cursor.execute("SELECT COUNT(*) FROM dc_section_similarities")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Load DC Code section similarities from NDJSON into database"
    )
    parser.add_argument(
        '--input',
        type=Path,
